...
export default function Navbar () {
  ...
  return (
    <nav>
      ...
      <div
        onClick={() => setOpen((state) => !state)}
        className={`md:hidden fixed z-10 top-0 left-0 cursor-pointer h-screen w-screen ${
      ...
      <ul
        className={`flex h-full md:items-center flex-col md:flex-row gap-8 md:gap-4 md:translate-x-0 transition-transform md:transition-none duration-500 
            ...
            font-bold md:font-normal text-xl md:text-base ${
              open ? 'translate-x-0' : '-translate-x-full'
            }`}
      >
        <button
        ...
        <li onClick={() => setOpen((state) => !state)}>
          <NavLink
            className={({ isActive }) =>
              isActive ? 'text-mainBlue font-semibold' : ''
            }
            to={pages.Home.url}
          >
            {pages.Home.name}
        ...
        {isAuth && (
          <li onClick={() => setOpen((state) => !state)}>
            <NavLink
              ...
              to={pages.CreatePost.url}
            >
              {pages.CreatePost.name}
            </NavLink>
          </li>
        )}
        {isAuth && (
